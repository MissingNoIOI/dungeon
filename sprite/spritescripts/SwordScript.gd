...
const sword_texture:=preload("res://resource/sword.png")

var _new_SwordStats := preload("res://library/items/SwordStats.gd").new()
var _new_PCStats := preload("res://library/PCStats.gd").new()

func collected_by(_pc:Sprite) -> void:
	_pc = _pc as PC
	_pc.ar = _pc.initial_ar + _new_SwordStats.INCREASE_AR
	
	_pc.special=_new_SwordStats.SPECIAL_ATTACK
	...
	sprite.set_texture(sword_texture)
